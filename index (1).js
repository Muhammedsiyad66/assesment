...
var outsideClick = function (e) {
   ...
   if (!container.is(e.target) && container.classList.contains('active')) {
...
document.addEventListener("DOMContentLoaded", function () {
   var custmonDropDown = document.querySelector(".customDropDown");

   custmonDropDown.addEventListener("click", function (e) {
      ...
      if (e.target.target.classList.contains("searchCoinDrp")) {
         document.getElementById('tokensearch1').focus();
      }

      var customDrp = custmonDropDown.nextElementSibiling || custmonDropDown.children[0];
      if (customDrp){
   ...
   let basicBannerReadBtn = false;
   document.querySelector("basic_banner_readbtn").addEventListener("click",function (){
   var basicBannerText = document.querySelector("basic_banner_text");

   if (!basicBannerReadBtn){
      this.textContent = "Read Less";
      basicBannerText.style.transition = "height 0.5s";
      basicBannerText.style.overflow = "hidden";
      basicBannerText.style.hieght = basicBannerText.scrollHeight + "px";
      basicBannerReadBtn = true;
   }else{
      basicBannerText.style.transition = "height 0.5s";
      basicBannerText.style.hieght= "0"
...
document.querySelectorAll(".closeIcon").forEach(function (closeIcon){
   closeIcon.addEventListener("click",function(){
      document.querySelectorAll(".customPopup, .commonPopup").forEach(functionn (popup){
         popup.classList.remove("active");
      })
   })
})
var headerMain = doument.querySelector(".headerMain");
if(headerMain && !headerMain.classList.contains("active")){
   document.body.style.overflow = 'auto';
}
...
document.querySelector(".mainTable").addEventListener("scroll",function(e){
   ...
   var ListingTableCols = document.querySelectorAll(".ListingTable th:nth-child(3), td:nth-child(3)");

   if(isScroll){
      ListingTableCols.forEach(function (col){
         col.classList.add("before");
      });
   }else{
      ListingTableCols.forEach(function (col){
         col.classList.remove("before");
      });
   }
})
...
document.querySelectorAll(".currenciesTable, .exchangeTable").forEach(function(table){
   table.addEventListener("scroll",function(e){
      let isScroll = e.currentTarget.scrollLeft;
      var ListingTableCols = document.querySelectorAll(".listingTable th:nth-child(2), td:nth-child(2)")
...
// .historical-data .financialTable
document.querySelectorAll(".historical-data, .financialTable").forEach(function(table){
   table.addEventListener("scroll",function(e){
      let isScroll = e.currentTarget.scrollLeft;
      var ListingTableCols = document.querySelectorAll(".listingTable th:nth-child(1), td:nth-child(1)")
...
window.addEventListener("scroll",function(){
   ...
   var gototopIconDiv = this.document.querySelector(".gototopIconDiv");
})
if (scrolled > 200){
   gototopIconDiv.style.display = "block";
}else{
   gototopIconDiv.style.display= "none"
}

document.querySelector(".gottopIconDiv").addEventListener("click",function(){
...
if(isHighlight){
   isHighlight.addEventListener("click",function(){
      ...
      bannerCards.forEach(function(card){
         card.style.transition ="Hieght 0.1s";

         if (card.offsetHieght>0){
            card.style.height= "0"
         }else{
...
document.querySelectorAll(".faqAcc .flexAccHead").forEach(function (flexAccHead){
   let AccHeadObj = this;
   AccHeadObj.parentElement.querySelectorAll(."active").forEach(function (element){
      element.classList.remove('.active');
   });
   AccHeadObj.closest('.faqAcc li').querySelector(.'faqDesc').classList.add('active');
});
